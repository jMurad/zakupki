...
class GuiClass(QtWidgets.QMainWindow, gui.Ui_MainWindow):
    ...
    def find(self):
        ...
        poisk.set_params(inputdata)
        poisk.get_all_pages(1, 50)
        self.progressBar.setMinimum(0)
        self.progressBar.setMaximum(poisk.totalPages)
        print(poisk.allPages[0])
        print(datetime.now())
        poisk.find_protocols()
...
class Poisk:
    ...
    allPages = []
    params = {
        'searchString': f'searchString={searchString}&',
        ...
        'orderNumber': f'orderNumber={orderNumber}&',
        'phase': ['af=on&', 'ca=on&', 'pc=on&', 'pa=on&'],
        'priceFromToGeneral': [f'priceFromGeneral={pfFrom}&', f'priceToGeneral={pfTo}&'],
        'currencyIdGeneral': 'currencyIdGeneral=-1&',
        'publishDateFromTo': [f'publishDateFrom={pdFrom}&', f'publishDateTo={pdTo}&'],
        'updateDateFromTo': [f'updateDateFrom={udFrom}&', f'updateDateTo={udTo}&'],
        'region_regions_': f'{regions}',
        'regions': f'regions={regions}',
        'regionDeleted': 'regionDeleted=false&',
        'sortBy': 'sortBy=UPDATE_DATE&',
        'exclText': f'exclText={exclText}'
    }
    ...
    def find_protocols(self):
        ...
        for page in self.allPages:
            ...
            if k == 0:
                ...
                self.callTotalResults(totalresults)
...
def main():
    ...
    window = GuiClass()  # Создаём объект класса GuiClass
    window.show()  # Показываем окно
    app.exec_()  # и запускаем приложение
'''
    poisk = Poisk()
    poisk.get_all_pages(2, 500)
    print(poisk.allPages[0])
    print(datetime.now())
    poisk.find_protocols()
    poisk.get_protocol_found(1)
